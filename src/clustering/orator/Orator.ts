...
import {
  IncomingMessage,
  LinkEvents,
  StateRehydratePayload,
} from '../link/ClusterLink';
...
export class Orator extends ClusterManager {
  constructor() {
    super({
      ...
      link: new ExchangeableLink({
        ...
        bootstrapPort: 1234, // Should not be necessary
      }),
    ...
    this.on(ClusterEvents.CONNECT_AS_NEW_COMPONENT, (msg: IncomingMessage) => {
      ...
      this.peers.getPeersOfComponent(comp).forEach((p) => {
        this._send(p.port, {
      ...
      this._send(msg.sender.port, {
        ...
        payload: this.peers.getPeersOfComponent(comp).map(
          (p) =>
            ({
              name: p.name,
              port: p.port,
    ...
    this.on(ClusterEvents.UNRESPONSIVE_COMPONENT, (name) => {
      ...
      this.peers.removeComponent(this.peers.getComponentByName(name.name));
      this.peers.getAllComponents().forEach((c) => {
        this._send(c.port, {
          type: ClusterEvents.STATE_REHYDRATE,
          payload: this.peers.getPeersOfComponent(c).map(
            (p) =>
              ({
                name: p.name,
                port: p.port,
  ...
  pingPeers() {
    this.peers
      .getAllComponents()
      .forEach((p) =>
        this._exchange(p.name, {}, LinkEvents.PING).then((r) =>
          VerboseLogging.info('responded with ', r)
        )
      );
  }
}
